...
import { generateHourlyData, getCongestionText, getFacilityLabel } from "@/lib/mock-data";
...
export function FacilityDetailPage({ facilityId }: FacilityDetailPageProps) {
	...
	useMetaDescription(
		facility
			? `${facility.name}의 실시간 혼잡도와 대기 현황을 확인하세요.`
			: "시설 상세 정보를 확인하세요.",
	);
			...
			<div className="max-w-lg mx-auto px-4 py-4 space-y-4 -mt-6 relative">
				<Card>
					<CardContent className="p-4">
						<div className="flex items-start justify-between gap-2">
							<div>
								<h1 className="text-xl font-bold mb-1" data-testid="text-facility-name">
									{facility.name}
								</h1>
				...
				<Card>
					<CardContent className="p-6 text-center">
						...
						<p className="text-2xl font-bold" data-testid="text-congestion-status">
							{getCongestionText(facility.avgCongestion)}
						</p>
					</CardContent>
				</Card>
						...
						<CardContent className="space-y-3">
							{facility.corners.map((corner) => (
								<div
									...
									data-testid={`seat-item-${corner.id}`}
								>
						...
						<CardContent className="space-y-3">
							{facility.corners.map((corner) => (
								<div
									key={corner.id}
									className="flex justify-between items-center p-3 bg-muted rounded-lg"
									data-testid={`gym-item-${corner.id}`}
								>
									<span className="font-medium">{corner.name}</span>
									<div className="flex items-center gap-3">
										<span className="text-sm">
