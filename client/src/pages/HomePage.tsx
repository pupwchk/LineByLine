...
import { getCurrentTimeSlot, TimeSlotSelector } from "@/components/TimeSlotSelector";
...
export function HomePage({ selectedDate, onNavigateToWaiting, onNavigateToDetail }: HomePageProps) {
	...
	useMetaDescription(
		"대학교 교내 시설의 실시간 혼잡도를 확인하고 대기 등록을 해보세요. 식당, 도서관, 헬스장 혼잡도를 한눈에.",
	);
	...
	const _isCurrentMealTime = isMealTime(selectedTime);
	const _currentMealPeriod = getMealPeriodName(selectedTime);
		...
		<div className="pb-20" data-testid="page-home">
			<div className="max-w-lg mx-auto px-4 py-4">
				{activeWaiting && isTodayDate && (
					<ActiveWaitingBanner waiting={activeWaiting} onClick={onNavigateToWaiting} />
				)}
				...
				{!isTodayDate && (
					<div
						className="mb-4 p-3 bg-muted rounded-lg text-center"
						data-testid="notice-future-date"
					>
						<p className="text-sm text-muted-foreground">
							{format(selectedDate, "M월 d일", { locale: ko })} 예측 정보입니다
						</p>
						<p className="text-xs text-muted-foreground mt-1">메뉴 정보는 당일에만 표시됩니다</p>
					</div>
				)}
